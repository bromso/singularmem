/* tslint:disable */
/* eslint-disable */
/* prettier-ignore */

/* auto-generated by NAPI-RS */

const { existsSync, readFileSync } = require('fs')
const { join } = require('path')

const { platform, arch } = process

let nativeBinding = null
let localFileExisted = false
let loadError = null

function isMusl() {
  // For Node 10
  if (!process.report || typeof process.report.getReport !== 'function') {
    try {
      const lddPath = require('child_process').execSync('which ldd').toString().trim()
      return readFileSync(lddPath, 'utf8').includes('musl')
    } catch (e) {
      return true
    }
  } else {
    const { glibcVersionRuntime } = process.report.getReport().header
    return !glibcVersionRuntime
  }
}

switch (platform) {
  case 'android':
    switch (arch) {
      case 'arm64':
        localFileExisted = existsSync(join(__dirname, 'singularmem.android-arm64.node'))
        try {
          if (localFileExisted) {
            nativeBinding = require('./singularmem.android-arm64.node')
          } else {
            nativeBinding = require('@jonasbroms/singularmem-android-arm64')
          }
        } catch (e) {
          loadError = e
        }
        break
      case 'arm':
        localFileExisted = existsSync(join(__dirname, 'singularmem.android-arm-eabi.node'))
        try {
          if (localFileExisted) {
            nativeBinding = require('./singularmem.android-arm-eabi.node')
          } else {
            nativeBinding = require('@jonasbroms/singularmem-android-arm-eabi')
          }
        } catch (e) {
          loadError = e
        }
        break
      default:
        throw new Error(`Unsupported architecture on Android ${arch}`)
    }
    break
  case 'win32':
    switch (arch) {
      case 'x64':
        localFileExisted = existsSync(
          join(__dirname, 'singularmem.win32-x64-msvc.node')
        )
        try {
          if (localFileExisted) {
            nativeBinding = require('./singularmem.win32-x64-msvc.node')
          } else {
            nativeBinding = require('@jonasbroms/singularmem-win32-x64-msvc')
          }
        } catch (e) {
          loadError = e
        }
        break
      case 'ia32':
        localFileExisted = existsSync(
          join(__dirname, 'singularmem.win32-ia32-msvc.node')
        )
        try {
          if (localFileExisted) {
            nativeBinding = require('./singularmem.win32-ia32-msvc.node')
          } else {
            nativeBinding = require('@jonasbroms/singularmem-win32-ia32-msvc')
          }
        } catch (e) {
          loadError = e
        }
        break
      case 'arm64':
        localFileExisted = existsSync(
          join(__dirname, 'singularmem.win32-arm64-msvc.node')
        )
        try {
          if (localFileExisted) {
            nativeBinding = require('./singularmem.win32-arm64-msvc.node')
          } else {
            nativeBinding = require('@jonasbroms/singularmem-win32-arm64-msvc')
          }
        } catch (e) {
          loadError = e
        }
        break
      default:
        throw new Error(`Unsupported architecture on Windows: ${arch}`)
    }
    break
  case 'darwin':
    localFileExisted = existsSync(join(__dirname, 'singularmem.darwin-universal.node'))
    try {
      if (localFileExisted) {
        nativeBinding = require('./singularmem.darwin-universal.node')
      } else {
        nativeBinding = require('@jonasbroms/singularmem-darwin-universal')
      }
      break
    } catch {}
    switch (arch) {
      case 'x64':
        localFileExisted = existsSync(join(__dirname, 'singularmem.darwin-x64.node'))
        try {
          if (localFileExisted) {
            nativeBinding = require('./singularmem.darwin-x64.node')
          } else {
            nativeBinding = require('@jonasbroms/singularmem-darwin-x64')
          }
        } catch (e) {
          loadError = e
        }
        break
      case 'arm64':
        localFileExisted = existsSync(
          join(__dirname, 'singularmem.darwin-arm64.node')
        )
        try {
          if (localFileExisted) {
            nativeBinding = require('./singularmem.darwin-arm64.node')
          } else {
            nativeBinding = require('@jonasbroms/singularmem-darwin-arm64')
          }
        } catch (e) {
          loadError = e
        }
        break
      default:
        throw new Error(`Unsupported architecture on macOS: ${arch}`)
    }
    break
  case 'freebsd':
    if (arch !== 'x64') {
      throw new Error(`Unsupported architecture on FreeBSD: ${arch}`)
    }
    localFileExisted = existsSync(join(__dirname, 'singularmem.freebsd-x64.node'))
    try {
      if (localFileExisted) {
        nativeBinding = require('./singularmem.freebsd-x64.node')
      } else {
        nativeBinding = require('@jonasbroms/singularmem-freebsd-x64')
      }
    } catch (e) {
      loadError = e
    }
    break
  case 'linux':
    switch (arch) {
      case 'x64':
        if (isMusl()) {
          localFileExisted = existsSync(
            join(__dirname, 'singularmem.linux-x64-musl.node')
          )
          try {
            if (localFileExisted) {
              nativeBinding = require('./singularmem.linux-x64-musl.node')
            } else {
              nativeBinding = require('@jonasbroms/singularmem-linux-x64-musl')
            }
          } catch (e) {
            loadError = e
          }
        } else {
          localFileExisted = existsSync(
            join(__dirname, 'singularmem.linux-x64-gnu.node')
          )
          try {
            if (localFileExisted) {
              nativeBinding = require('./singularmem.linux-x64-gnu.node')
            } else {
              nativeBinding = require('@jonasbroms/singularmem-linux-x64-gnu')
            }
          } catch (e) {
            loadError = e
          }
        }
        break
      case 'arm64':
        if (isMusl()) {
          localFileExisted = existsSync(
            join(__dirname, 'singularmem.linux-arm64-musl.node')
          )
          try {
            if (localFileExisted) {
              nativeBinding = require('./singularmem.linux-arm64-musl.node')
            } else {
              nativeBinding = require('@jonasbroms/singularmem-linux-arm64-musl')
            }
          } catch (e) {
            loadError = e
          }
        } else {
          localFileExisted = existsSync(
            join(__dirname, 'singularmem.linux-arm64-gnu.node')
          )
          try {
            if (localFileExisted) {
              nativeBinding = require('./singularmem.linux-arm64-gnu.node')
            } else {
              nativeBinding = require('@jonasbroms/singularmem-linux-arm64-gnu')
            }
          } catch (e) {
            loadError = e
          }
        }
        break
      case 'arm':
        if (isMusl()) {
          localFileExisted = existsSync(
            join(__dirname, 'singularmem.linux-arm-musleabihf.node')
          )
          try {
            if (localFileExisted) {
              nativeBinding = require('./singularmem.linux-arm-musleabihf.node')
            } else {
              nativeBinding = require('@jonasbroms/singularmem-linux-arm-musleabihf')
            }
          } catch (e) {
            loadError = e
          }
        } else {
          localFileExisted = existsSync(
            join(__dirname, 'singularmem.linux-arm-gnueabihf.node')
          )
          try {
            if (localFileExisted) {
              nativeBinding = require('./singularmem.linux-arm-gnueabihf.node')
            } else {
              nativeBinding = require('@jonasbroms/singularmem-linux-arm-gnueabihf')
            }
          } catch (e) {
            loadError = e
          }
        }
        break
      case 'riscv64':
        if (isMusl()) {
          localFileExisted = existsSync(
            join(__dirname, 'singularmem.linux-riscv64-musl.node')
          )
          try {
            if (localFileExisted) {
              nativeBinding = require('./singularmem.linux-riscv64-musl.node')
            } else {
              nativeBinding = require('@jonasbroms/singularmem-linux-riscv64-musl')
            }
          } catch (e) {
            loadError = e
          }
        } else {
          localFileExisted = existsSync(
            join(__dirname, 'singularmem.linux-riscv64-gnu.node')
          )
          try {
            if (localFileExisted) {
              nativeBinding = require('./singularmem.linux-riscv64-gnu.node')
            } else {
              nativeBinding = require('@jonasbroms/singularmem-linux-riscv64-gnu')
            }
          } catch (e) {
            loadError = e
          }
        }
        break
      case 's390x':
        localFileExisted = existsSync(
          join(__dirname, 'singularmem.linux-s390x-gnu.node')
        )
        try {
          if (localFileExisted) {
            nativeBinding = require('./singularmem.linux-s390x-gnu.node')
          } else {
            nativeBinding = require('@jonasbroms/singularmem-linux-s390x-gnu')
          }
        } catch (e) {
          loadError = e
        }
        break
      default:
        throw new Error(`Unsupported architecture on Linux: ${arch}`)
    }
    break
  default:
    throw new Error(`Unsupported OS: ${platform}, architecture: ${arch}`)
}

if (!nativeBinding) {
  if (loadError) {
    throw loadError
  }
  throw new Error(`Failed to load native binding`)
}

const { Store: _NativeStore, version, PlainAdapter, ClaudeAdapter, OpenAiAdapter, GeminiAdapter } = nativeBinding

/**
 * Convert an Item from the native binding into a JS-friendly shape:
 * `createdAt` is promoted from a number (ms since epoch) to a `Date`.
 */
function liftItem(raw) {
  return Object.assign(Object.create(null), raw, { createdAt: new Date(raw.createdAt) })
}

/** Public Store class — thin wrapper that promotes `createdAt` to `Date`. */
class Store {
  /** @private */
  constructor(native) {
    this._native = native
  }

  static open(path, options) {
    return _NativeStore.open(path, options).then((native) => new Store(native))
  }

  get(id) {
    return this._native.get(id).then(liftItem)
  }

  list(options) {
    return this._native.list(options).then((items) => items.map(liftItem))
  }

  revisions(id) {
    return this._native.revisions(id).then((items) => items.map(liftItem))
  }

  search(query, options) {
    return this._native.search(query, options).then((res) => ({
      query: res.query,
      hits: res.hits.map((h) => ({ ...h, item: liftItem(h.item) })),
    }))
  }

  retrieve(query, options) {
    return this._native.retrieve(query, options).then((ctx) => ({
      query: ctx.query,
      blocks: ctx.blocks.map((b) => ({ ...b, createdAt: new Date(b.createdAt) })),
    }))
  }

  ingest(item) {
    return this._native.ingest(item).then(liftItem)
  }

  formatVersion() {
    return this._native.formatVersion()
  }

  export() {
    return this._native.export()
  }
}

// Construct the frozen `adapters` namespace from the four native classes.
const adapters = Object.freeze({
  plain:  Object.freeze(new PlainAdapter()),
  claude: Object.freeze(new ClaudeAdapter()),
  openai: Object.freeze(new OpenAiAdapter()),
  gemini: Object.freeze(new GeminiAdapter()),
})
module.exports.adapters = adapters
module.exports.Store = Store
module.exports.version = version
