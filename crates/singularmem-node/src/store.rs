...
use singularmem_core::{ScopeFilter, Store as CoreStore, StoreOptions as CoreStoreOptions};
...
pub struct ListTask {
    ...
    /// Pre-set validation error (e.g. malformed `scope`). When `Some`,
    /// `compute` immediately fails so the Promise rejects rather than the
    /// method throwing synchronously (mirrors `GetTask::pre_error`).
    pre_error: Option<NapiError<&'static str>>,
    failed: Option<NodeError>,
}
...
impl Task for ListTask {
    ...
    fn compute(&mut self) -> napi::Result<Self::Output> {
        ...
        let result: singularmem_core::Result<Vec<singularmem_core::Item>> = (|| {
            ...
            let iter = self
                .store
                .list_by_tags_scoped(&refs, self.filter.as_ref())?;
            let mut out = Vec::new();
            for item in iter {
                out.push(item?);
...
/// Build a `ScopeFilter` from the JS-facing `scope`/`scopeExact` option pair.
/// `scope: None` means "no filtering" and returns `Ok(None)`. Validation
/// failures (bad segment, too many segments, etc.) map through `NodeError`
/// to a `NapiError<&'static str>` with `.code === "Validation"`.
///
/// # Errors
///
...
fn scope_filter(
    ...
    let filter = if exact.unwrap_or(false) {
        ScopeFilter::exact(&path)
    } else {
        ScopeFilter::descendants(&path)
    };
    filter.map(Some).map_err(|e| {
...
///
/// When `scope` is set, the store is attached as the searcher's
/// `ScopeLookup` (needed to post-filter semantic hits); harmless to attach
/// for lexical-only searches, which don't consult it.
///
/// # Errors
///
...
fn run_search(
    ...
    let searcher = if scope.is_some() {
        searcher.with_scope_lookup(store.as_ref())
    } else {
        searcher
    };
...
fn run_retrieve(
    ...
    let opts = singularmem_retrieve::RetrieveOptions {
        ...
        // `Retriever::retrieve` copies this onto `search.scope` and, when
        // set, auto-attaches `store` as the `ScopeLookup` if the searcher
        // doesn't already have one — no explicit `with_scope_lookup` call
        // needed here (unlike `run_search`, which builds the searcher
        // directly).
        scope,
    };
    retriever
...
pub struct ScopesTask {
    store: Arc<CoreStore>,
    failed: Option<NodeError>,
}

#[napi]
impl Task for ScopesTask {
    type Output = Vec<(String, usize)>;
    type JsValue = Vec<crate::types::ScopeCount>;

    fn compute(&mut self) -> napi::Result<Self::Output> {
        match self.store.scopes() {
            Ok(rows) => Ok(rows),
            Err(e) => {
                self.failed = Some(NodeError::from(e));
                Err(NapiError::new(
...
pub struct SetScopeTask {
    store_path: PathBuf,
    read_only: bool,
    /// `None` when `id` failed to parse; `compute` short-circuits via
    /// `pre_error` in that case, so the value is never used.
    id: Option<ItemId>,
    scope: Option<String>,
    pre_error: Option<NapiError<&'static str>>,
    failed: Option<NodeError>,
...
impl Task for SetScopeTask {
    ...
    fn compute(&mut self) -> napi::Result<Self::Output> {
        ...
        let id = self.id.expect("id must be Some when pre_error is None");
        match run_set_scope(&self.store_path, self.read_only, id, self.scope.take()) {
            Ok(item) => Ok(item),
            Err(e) => {
                self.failed = Some(NodeError::from(e));
                Err(NapiError::new(
...
/// Open a fresh writable `CoreStore` at `store_path` and move `id` to
/// `scope` (`None` clears it). Mirrors `IngestTask`'s per-call store-open
/// pattern; unlike ingest, `set_scope` does not fire index hooks (the
/// Tantivy document keeps its old scope until `singularmem reindex`), so no
/// sidecar wiring is needed here.
///
/// # Errors
///
/// Propagates `Error::ReadOnly`, `Error::Validation`, `Error::NotFound`, or
/// `Error::Sqlite` from `singularmem_core::Store::set_scope`.
fn run_set_scope(
    store_path: &std::path::Path,
    read_only: bool,
    id: ItemId,
    ...
    let store = CoreStore::open_with_options(store_path, CoreStoreOptions { read_only })?;
    store.set_scope(id, scope.as_deref())
}
...
impl Store {
    ...
    pub fn list(&self, options: Option<ListOptions>) -> napi::Result<AsyncTask<ListTask>> {
        ...
        let (tags, limit, scope_str, scope_exact) = options
            .map(|o| {
                (
                    o.tags.unwrap_or_default(),
                    o.limit.map(|n| n as usize),
                    o.scope,
                    o.scope_exact,
                )
            })
            .unwrap_or_default();
        let (filter, pre_error) = match scope_filter(scope_str, scope_exact) {
            Ok(f) => (f, None),
            Err(e) => (None, Some(e)),
        };
    ...
    pub fn search(
        ...
        let (mode, mode_err) = match SearchMode::from_optional_str(mode_str) {
            Ok(m) => (m, None),
            Err(coded) => (SearchMode::Auto, Some(coded)),
        };

        let scope_str = options.as_ref().and_then(|o| o.scope.clone());
        let scope_exact = options.as_ref().and_then(|o| o.scope_exact);
        let (scope, scope_err) = match scope_filter(scope_str, scope_exact) {
            Ok(s) => (s, None),
            Err(coded) => (None, Some(coded)),
        };
        let pre_error = mode_err.or(scope_err);
    ...
    #[napi]
    #[allow(clippy::missing_errors_doc)]
    pub fn scopes(&self) -> napi::Result<AsyncTask<ScopesTask>> {
        Ok(AsyncTask::new(ScopesTask {
            store: self.inner.clone(),
            failed: None,
        }))
    }
    ...
    pub fn set_scope(
        ...
        id: String,
        scope: Option<String>,
    ) -> napi::Result<AsyncTask<SetScopeTask>> {
        let (item_id, pre_error) = match ItemId::from_str(&id) {
            Ok(id) => (Some(id), None),
            Err(e) => {
                let core_err = singularmem_core::Error::from(e);
