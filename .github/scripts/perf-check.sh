...
# indexes, 15=single ingest with indexes, 16=query, 17=semantic, 18=hybrid.
...
# 4c. Single-item ingest with both indexes attached, at 20,000 pre-seeded
# vectors: <= 20 ms. Asserts the journal makes per-item commit cost
# independent of index size (no more whole-graph rewrite per ingest).
# bench path: target/criterion/ingest_throughput/ingest_single_with_indexes/new/estimates.json
...
echo "  single ingest w/ index: ${SINGLE_MS} ms (limit 20)"
